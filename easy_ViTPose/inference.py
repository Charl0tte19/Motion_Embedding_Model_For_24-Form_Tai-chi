...
import pdb
from .vit_utils.post_processing import get_affine_transform
...
# 這是我加的
# 把原影片的frame，依照bbox裁切，並縮放成model的input size
def test_transform(src_img, bbox):
    ...
    center, scale = _box_to_center_scale(
        xmin, ymin, xmax - xmin, ymax - ymin, 3/4)
    scale = scale * 1.0
    input_size = (256,192)
...
class VitInference:
    ...
    def __init__(self, model: str,
        ...
        # Size of the input image for YOLOv8 model. Defaults to 320.
        self.yolo_size = yolo_size
        # The tracker can be used to predict the bboxes instead of yolo for performance
        ...
        if model_name is None:
            ...
            '''
            model_cfg是根據我們的model_name去取的，比如這個例子裡就是base model:
            {'type': 'TopDown', 'pretrained': None, 'backbone': {'type': 'ViT', 'img_size': (256, 192), 'patch_size': 16, 'embed_dim': 768, 'depth': 12, 'num_heads': 12, 'ratio': 1, 'use_checkpoint': False, 'mlp_ratio': 4, 'qkv_bias': True, 'drop_path_rate': 0.3}, 'keypoint_head': {'type': 'TopdownHeatmapSimpleHead', 'in_channels': 768, 'num_deconv_layers': 2, 'num_deconv_filters': (256, 256), 'num_deconv_kernels': (4, 4), 'extra': {'final_conv_kernel': 1}, 'loss_keypoint': {'type': 'JointsMSELoss', 'use_target_weight': True}, 'out_channels': 133}, 'train_cfg': {}, 'test_cfg': {'flip_test': True, 'post_process': 'default', 'shift_heatmap': False, 'target_type': 'GaussianHeatmap', 'modulate_kernel': 11, 'use_udp': True}}
            跟dataset的設定關係不大
            '''
    ...
    @classmethod
    def postprocess(cls, heatmaps, org_w, org_h, bbox, origin_size=False):
        """
        ...
        """
        # > /home/charl0tte/easy_ViTPose/easy_ViTPose/vit_utils/top_down_eval.py(493)keypoints_from_heatmaps()
        
        if origin_size:
            # 這是我加的，生成的keypoint的座標是基於原始影片的size
            scale = np.array([[bbox[2]-bbox[0],bbox[3]-bbox[1]]])
            center = np.array([[bbox[0]+scale[0][0]*0.5,bbox[1]+scale[0][1]*0.5]])
        ...
        # points: shape是(1,133,2)，prob: shape是(1,133,1)
        # return np.concatenate([points[:, :, ::-1], prob], axis=2)
        # 根據上行，vitpose把x和y的維度位置交換了
        # 但我不想交換，於是變下行
    ...
    def inference(self, img: np.ndarray, origin_size=False) -> dict[typing.Any, typing.Any]:
        """
        ...
        if (self.tracker is None or
           (self.frame_counter % self.yolo_step == 0 or self.frame_counter < 3)):
            ...
            # 取佔畫面最大的那個人的框框
            res_xywh = results.boxes.xywh
            if(res_xywh.numel()>0):
                boxes_areas = res_xywh[:, 2] * res_xywh[:, 3]
                max_box_index = torch.argmax(boxes_areas)

                # res_pd = np.array([r[:5].tolist() for r in  # TODO: Confidence threshold
                #                    results.boxes.data.cpu().numpy() if r[4] > 0.35]).reshape((-1, 5))

                res_data = results.boxes.data.cpu().numpy()
                # Confidence threshold
                if res_data[max_box_index,4] > 0.35:
                    res_pd = np.array(res_data[max_box_index,:5].tolist()).reshape((-1, 5))
                else:
                    res_pd = np.array([]).reshape((-1, 5))
        ...
        for bbox, id in zip(bboxes, ids):
            bbox_empty = False
            img_inf, bbox = test_transform(img, bbox)

            keypoints, img_resize = self._inference(img_inf, bbox, origin_size)
            
            frame_keypoints[id] = keypoints

        
        # 存下來是為了在draw的時候使用
        if (not bbox_empty) and self.save_state:
            if origin_size:
                self._img = img
            else:
                self._img = img_resize
            self._yolo_res = results
            self._tracker_res = (bboxes, ids, scores)
            self._keypoints = frame_keypoints

        return frame_keypoints, bbox_empty



    def draw(self, keypoints, show_yolo=True, show_raw_yolo=False, confidence_threshold=0.5):
        """
    ...
    # 我加的，主要是直接指定背景圖，不用self._img
    def draw_with_img(self, keypoints, origin_img, show_yolo=True, show_raw_yolo=False, confidence_threshold=0.5):
        """
        Draw keypoints and bounding boxes on the image.
        ...
        for idx, k in keypoints.items():
            img = draw_points_and_skeleton(img.copy(), k,
                                           joints_dict()[self.dataset]['keypoints'],
                                           joints_dict()[self.dataset]['skeleton'], # edge怎麼畫
                                           person_index=idx,
                                           points_color_palette='gist_rainbow',
                                           skeleton_color_palette='jet',
                                           points_palette_samples=10,
    ...
    def pre_img(self, img):
        ...
        # 在warpAffine()已經變成192x256了
        # 所以img和img_resize根本一樣，org_h和org_w也根本不是原圖大小
        img_resize = img
        img_input = img_resize / 255
        img_input = ((img_input - MEAN) / STD).transpose(2, 0, 1)[None].astype(np.float32)
        return img_input, org_h, org_w, img_resize

    @torch.no_grad()
    def _inference_torch(self, img: np.ndarray, bbox=None, origin_size=False) -> np.ndarray:
        # Prepare input data
        img_input, org_h, org_w, img_resize = self.pre_img(img)
        img_input = torch.from_numpy(img_input).to(torch.device(self.device))

        # Feed to model
        heatmaps = self._vit_pose(img_input).detach().cpu().numpy()
        return self.postprocess(heatmaps, img_resize.shape[1], img_resize.shape[0], bbox, origin_size)[0], img_resize
