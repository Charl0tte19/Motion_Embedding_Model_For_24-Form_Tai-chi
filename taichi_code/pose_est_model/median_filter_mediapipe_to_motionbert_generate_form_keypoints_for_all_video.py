...
def landmark2ndarray(landmarks):
...
def ndarray2landmark(filtered_ndarray):
...
def draw_2d_landmarks(rgb_image, detection_result):
...
def process_video(input_file=None, output_path=None, motionbert_model=None, args=None, fps_in=10):
    ...
    if is_video:
        ...
        if args.save_video:
            ...
            out_writer_2d = cv2.VideoWriter(output_path_video_3d_pose,
                                        #cv2.VideoWriter_fourcc(*'vp09'),
                                        cv2.VideoWriter_fourcc(*'mp4v'),
                                        fps_in, (frame_width, frame_height))  # type: ignore

            out_writer_3d_world = cv2.VideoWriter(output_path_video_3d_world,
                                            #cv2.VideoWriter_fourcc(*'vp09'),
                                            cv2.VideoWriter_fourcc(*'mp4v'),
                                            fps_in, (400,400))  # type: ignore

            out_writer_3d_motionbert = cv2.VideoWriter(output_path_video_3d,
                                            #cv2.VideoWriter_fourcc(*'vp09'),
                                            cv2.VideoWriter_fourcc(*'mp4v'),
                                            fps_in, (400,400))  # type: ignore
        ...
        while cap.grab():
            ...
            mediapipe_imgs.append(mp_image)
            mediapipe_result_ndarrays.append(landmark2ndarray(pose_landmarker_result))
        ...
        # shape (frames_size, 2, 33, 5)
        mediapipe_result_ndarrays=np.array(mediapipe_result_ndarrays, dtype=np.float32)
        mediapipe_result_ndarrays[..., :-2] = bidirectional_filter(data=mediapipe_result_ndarrays[...,:-2], window_size=5, filter='median')
        pose_landmarker_results_list = ndarray2landmark(mediapipe_result_ndarrays)
        
        for f in range(len(pose_landmarker_results_list)):
            mp_image = mediapipe_imgs[f]
            if args.save_video:
                img_2d_pose = draw_2d_landmarks(mp_image.numpy_view(), pose_landmarker_results_list[f])
                img_3d_world = draw_3d_landmarks(pose_landmarker_results_list[f], fig, ax)
                out_writer_2d.write(img_2d_pose)
                out_writer_3d_world.write(img_3d_world)
            
            image_height, image_width, _ = mp_image.numpy_view().shape
            pose_landmarks = pose_landmarker_results_list[f].pose_landmarks[0]
            pose_world_landmarks = pose_landmarker_results_list[f].pose_world_landmarks[0]
        ...
        np.savez(output_path_3d_pose_npz, frame_ith=np.array(frame_ith, dtype=np.int32), keypoints=keypoints)
        print('>>> Saved keypoints 3d_pose npz')

        np.savez(output_path_3d_world_npz, frame_ith=np.array(frame_ith, dtype=np.int32), keypoints=keypoints_3d)
        print('>>> Saved keypoints 3d_world npz')

        wild_dataset = WildDetDataset(keypoints, clip_len=243, vid_size=vid_size, scale_range=None, focus=None, data_type="mediapipe")
        ...
        if args.save_video:
            ...
            for img_3d in output_imgs_3d:
                out_writer_3d_motionbert.write(img_3d)
            out_writer_3d_motionbert.release()
