...
def dataset_collate_fn(batch, form_file_paths_dict, cfg):
    ...
    anchor_clip_lens = []
    for i in range(len(anchors_clips)):
        anchor_clip_lens.append(len(anchors_clips[i]))
    ...
    if cfg.kpts_type_for_train=="motionbert":
        return torch.FloatTensor(apn_pairs_with_multiple_data_views), torch.FloatTensor(ap_similarity), torch.FloatTensor(an_similarity), anchor_clip_lens
